...
仅 **框架运行时表**（`framework_*`），定义于 `sdkwork-web-store-sqlx`（可选 crate）。

| 包含 | 不包含 |
| --- | --- |
| `framework_rate_limit_policy` | `iam_*` |
| `framework_idempotency_record` | 产品业务表 |
| `framework_audit_event`（框架审计通道） | IAM 登录会话表 |
| `framework_security_event` | |
| `framework_cors_policy` | |
| `framework_tenant_runtime_profile` | |
...
### framework_rate_limit_policy

租户/平台级流控策略模板。见前版 §3.1 字段定义。

### framework_idempotency_record
...
pub trait AuditEmitter: Send + Sync { ... }  // 可落 framework_audit_event 或 noop
```

- `MemoryRateLimitStore` → `sdkwork-web-core`
- `SqlxRateLimitStore` → `sdkwork-web-store-sqlx`（仅 `framework_*`）
...
- [x] migration 仅含 `framework_*`（`sqlx_migrations` architecture test）
