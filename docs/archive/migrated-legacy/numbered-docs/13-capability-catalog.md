...
> 前置：[12-industry-framework-benchmark.md](./12-industry-framework-benchmark.md)  
> 成熟度等级见 [16-maturity-model.md](./16-maturity-model.md)（M0–M4）。
...
| F3 | contract_fallback | 501/404 Problem+json + requestId/traceId；`service_router` fallback + `WebFrameworkBuilder::route_manifest` | bootstrap | M3 |
...
| F7 | 响应信封区分 | Problem vs PlusApiResult | error mapping 文档 | M3 |
...
| ID | 功能点 | 技术点 | 框架模块 | 目标 M |
| --- | --- | --- | --- | --- |
| G1 | Problem+json | RFC 7807 + requestId/traceId；`WebFrameworkRejection` / `finish_api_json` | context: WebFrameworkError | M3 |
...
- [14-standards-system.md](./14-standards-system.md)
- [15-extension-points-registry.md](./15-extension-points-registry.md)
